		...
		if(sw.pollSkeleton()){
			for( int ii = 0; ii < (int)Kinect.NuiSkeletonPositionIndex.Count; ii++)
			{
				if( ((uint)Mask & (uint)(1 << ii) ) > 0 && (_nullMask & (uint)(1 << ii)) <= 0 )
				{
					...
					RotateJoint(ii);
					mainbody.position=new Vector3((float)(sw.bonePos[player,(int)Kinect.NuiSkeletonPositionIndex.HipCenter].x*10),mainbody.position.y,(float)(sw.bonePos[player,(int)Kinect.NuiSkeletonPositionIndex.HipCenter].z*20));
				}
			}
		}	
		/*		
